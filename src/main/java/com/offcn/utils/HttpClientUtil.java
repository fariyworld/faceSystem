...
public class HttpClientUtil {
	...
	public static <T> T doPost(Class<T> clazz, String url) {
		...
		try {
			...
			System.out.println("服务器返回状态码：" + status.getStatusCode());
	...
	public static <T> T doPost(String url, T t, Map<String, String> headerMap) {
		...
		try {
			...
			if (status.getStatusCode() == 200) {
				...
				return (T) JSONObject.toJavaObject(jsonObject, t.getClass());
			}
	...
	public static ResultBean toResultBean(String result) {

		String jsonStr = "{\"result\":" + result + "}";
		
		JSONArray jsonArray = JSON.parseObject(jsonStr).getJSONArray("result");

		return jsonArray.getObject(0, ResultBean.class);
	}
}
